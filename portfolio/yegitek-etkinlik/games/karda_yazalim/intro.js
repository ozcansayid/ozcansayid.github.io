/**
 * Karda Yazalım - Intro Overlay Manager
 * Handles the initial full-screen overlay, audio instructions, and "How to Play" functionality.
 * Adapted from Harfi Yazalım Intro.
 */
const KardaYazalimIntro = {
    overlayId: 'karda-yazalim-intro-overlay',
    // User requested same intro style. Using Harfi Yazalim instruction as fallback or specific one if found.
    // Based on file search, if karda_yazalim_instruction.mp3 exists, I will use that. 
    // For now defaulting to the one used in index.html or harfi_yazalim if requested.
    // Viewing index.html (line 459) shows: 'harfi_yazalim_instruction.mp3'.
    // So I will use that as per the existing code in index.html.
    audioPath: '../../assets/audio/harfi_yazalim_instruction.mp3',
    audio: null,
    isPlaying: false,

    init: function () {
        console.log("KardaYazalimIntro: Initializing...");
        this.createOverlay();
        this.setupAudio();
        this.show();
    },
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => KardaYazalimIntro.init(), 100);
});
