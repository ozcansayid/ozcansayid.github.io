/**
 * Karda Yazalım - Intro & Instruction Overlay
 */
window.KardaYazalimIntro = {
    overlayId: 'karda-yazalim-intro-overlay',
    audio: null,
    isPlaying: false,
    data: null,
    ...
    createOverlay: function () {
        ...
        const GAME_ASSETS = '../games/karda_yazalim/assets/';
        const gifPath = window.ASSETS_PATH + 'img/' + this.data.folderPrefix + '/' + this.data.images.writing;

        overlay.innerHTML = `
            <style>
                .demo-stage-snow { position:relative; width:600px; height:380px; background:url('${GAME_ASSETS}img/bg.jpg') no-repeat center center; background-size:cover; border:4px solid #fff; border-radius:20px; box-shadow:0 10px 30px rgba(0,0,0,0.3); overflow:hidden; display:flex; align-items:center; justify-content:center; }
                .demo-gif-snow { max-width:90%; max-height:90%; border-radius:15px; box-shadow:0 5px 15px rgba(0,0,0,0.2); }
            </style>
            <div class="intro-content text-center" style="max-width:800px;width:90%;">
                ...
                <div class="demo-stage-snow mb-4 mx-auto">
                    <img src="${gifPath}" class="demo-gif-snow" alt="Yazım Animasyonu">
                </div>
                <div class="actions mt-2">
                    <button id="btn-start-karda" class="btn btn-lg px-5 py-3 fw-bold shadow-lg"
        ...
        const btn = document.getElementById('btn-start-karda');
        if (btn) btn.onclick = (e) => { e.stopPropagation(); this.hide(); };
        // Removed overlay-level onclick to prevent unintended audio replay after hiding
    },

    setupAudio: function () {
        if (this.audio) return;
        const audioPath = '../games/karda_yazalim/assets/audio/' + (this.data.audio.instruction || 'instruction.mp3');
        this.audio = new Audio(audioPath);
        const btn = document.getElementById('btn-start-karda');
        this.audio.onended = () => {
            if (btn) { btn.disabled=false; btn.style.backgroundColor='#2ec4b6'; btn.style.cursor='pointer'; }
        };
        setTimeout(() => { if (btn && btn.disabled) { btn.disabled=false; btn.style.backgroundColor='#2ec4b6'; } }, 10000);
    },
...
window.KardaYazalimInstruction = {
    init: function () {
        window.KardaYazalimIntro.createOverlay();
        const btn = document.getElementById('btn-start-karda');
        if (btn) {
