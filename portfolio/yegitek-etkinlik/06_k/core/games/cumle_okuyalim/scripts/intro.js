 ...
 */
window.CumleOkuyalimIntro = {
    overlayId: 'cumle-okuyalim-intro-overlay',
    audio: null,
    isPlaying: false,
    animRunning: false,
    data: null,
    ...
    createOverlay: function () {
        ...
        const sentenceStr = this.data && this.data.levels && this.data.levels[0]
            ? this.data.levels[0].text
            : 'in in in';
        const demoWords = sentenceStr.split(' ');
    ...
    setupAudio: function () {
        ...
        const audioPath = '../games/cumle_okuyalim/assets/audio/' + (this.data.audio?.instruction || 'instruction.mp3');
...
window.CumleOkuyalimInstruction = {
    init: function () {
        const existing = document.getElementById(window.CumleOkuyalimIntro.overlayId);
        if (existing) existing.remove();
        window.CumleOkuyalimIntro.stopDemoAnimation();

        window.CumleOkuyalimIntro.createOverlay();
        ...
        // Play audio
        if (!window.CumleOkuyalimIntro.audio) {
            const audioPath = '../games/cumle_okuyalim/assets/audio/' + (window.CumleOkuyalimIntro.data.audio?.instruction || 'instruction.mp3');
            window.CumleOkuyalimIntro.audio = new Audio(audioPath);
        }
        window.CumleOkuyalimIntro.audio.currentTime = 0;
        window.CumleOkuyalimIntro.audio.play().catch(() => {});

        window.CumleOkuyalimIntro.animRunning = true;
        window.CumleOkuyalimIntro.startDemoAnimation();
    },

    hide: function () {
        if (window.CumleOkuyalimIntro.audio) { window.CumleOkuyalimIntro.audio.pause(); window.CumleOkuyalimIntro.audio.currentTime = 0; }
        window.CumleOkuyalimIntro.stopDemoAnimation();

        const overlay = document.getElementById(window.CumleOkuyalimIntro.overlayId);
        if (overlay) {
            overlay.style.transition = 'opacity 0.5s ease';
            overlay.style.opacity = '0';
