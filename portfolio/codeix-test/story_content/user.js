    ...
    .then(data => {
        ...
        if (data.values && data.values[0]) {
            ...
            if (addPointValue && addPointValue.trim() !== "") {
                ...
                player.SetVar("addPointValue", "9");  // Boş olarak ayarla
            }
			
            // correctFeedback'i kontrol et ve gerçekten boşsa hiç set etme veya boş ata
            let correctFeedback = data.values[0][7];
            ...
            let wrongFeedback = data.values[0][8];
...
{
  // E-tablolardan gelen dinamik görsel URL'sini Storyline'a atıyoruz
let player = GetPlayer();
let gorselUrl = player.GetVar("image");  // E-tablodan gelen dinamik görsel URL'si

// Benzersiz data-acc-text ile üst elemanı seç
const container = document.querySelector('[data-acc-text="placeholder.png"]');

// SVG içindeki <image> tag'ini bul
if (container) {
    const svgImage = container.querySelector('image');
    
    if (svgImage) {
        svgImage.style.display = 'none';
        setTimeout(() => svgImage.style.display = 'block', 0);

        // Görselin kaynağını değiştir
        svgImage.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", gorselUrl);
    } else {
        console.warn("SVG içindeki <image> etiketi bulunamadı.");
    }
} else {
    console.warn("Belirtilen data-acc-text'e sahip görsel bulunamadı.");
}
